...
void c_tabs::render( ImVec2 const& panel_pos, float const panel_w, float const panel_h, float const alpha, float const delta_time, bool const can_interact, ImFont* font_medium_32, ImFont* icon_font )
{
    ...
    for ( int i = 0; i < k_tab_count; ++i )
    {
        ...
        {
            ImVec2 const icon_sz = icon_font->CalcTextSizeA( 16.0f, FLT_MAX, 0.0f, tabs[ i ].m_icon );
            ImVec2 const icon_pos = ImVec2( tab_min.x + 18.0f, tab_min.y + ( tab_h - icon_sz.y ) * 0.5f );
            draw_list->AddText( icon_font, 16.0f, icon_pos, item_col, tabs[ i ].m_icon );
        }

        if ( font_medium_32 != nullptr )
        {
            ImVec2 const text_pos = ImVec2( tab_min.x + 48.0f, tab_min.y + ( tab_h - 20.0f ) * 0.5f );
            draw_list->AddText( font_medium_32, 20.0f, text_pos, item_col, tabs[ i ].m_name );
        }
        ...
        if ( expand_val > 0.001f )
        {
            ...
            for ( int j = 0; j < sub_count; ++j )
            {
                ...
                if ( sub_alpha > 0.001f )
                {
                    ...
                    if ( font_medium_32 != nullptr )
                    {
                        ImVec2 const sub_txt_pos = ImVec2( sub_min.x + 18.0f, sub_min.y + ( sub_tab_h - 20.0f ) * 0.5f );
                        draw_list->AddText( font_medium_32, 20.0f, sub_txt_pos, sub_txt_col, tabs[ i ].m_sub_tabs[ j ].m_name );
                    }
                }
            }
